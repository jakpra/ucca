...
CONSTRUCTIONS = (
    ...
    Construction("Non-preterm", "Non-preterm", lambda c: all(not t.startswith('Preterminal') for t in c.edge.tags) and Candidate.is_primary(c)),
    Construction("preterminals", "preterminals", lambda c: any(t.startswith('Preterminal') for t in c.edge.tags) and Candidate.is_primary(c)),
    Construction("has_tags", "has_tags", lambda c: any(c.edge.tags) and Candidate.is_primary(c)),
    Construction("SNACS", "SNACS", lambda c: any(any(_t.startswith('p.') for _t in t.split(':')) for t in c.edge.tags) and Candidate.is_primary(c)),
    Construction("has_gold_SNACS", "has_gold_SNACS", lambda c: c.edge.child.extra.get('has_gold_ss') if not c.edge.child.outgoing else any(ter.extra.get('has_gold_ss') for ter in c.edge.child.terminals) and Candidate.is_primary(c)),
    Construction("has_gold_SNACS_sibling", "has_gold_SNACS_sibling",
                 lambda c: any(sib.child.extra.get('has_gold_ss') if not sib.child.outgoing else any(ter.extra.get('has_gold_ss') for ter in sib.child.terminals) for sib in c.edge.parent.outgoing if sib.child != c.edge.child) and Candidate.is_primary(c)),
    Construction("has_gold_SNACS_or_sibling", "has_gold_SNACS_or_sibling",
                 lambda c: any(sib.child.extra.get('has_gold_ss') if not sib.child.outgoing else any(ter.extra.get('has_gold_ss') for ter in sib.child.terminals) for sib in c.edge.parent.outgoing) and Candidate.is_primary(c)),
    Construction("scenes", "scenes", lambda c: c.edge.child.is_scene() and Candidate.is_primary(c)),
    Construction("scene_children", "scene_children", lambda c: c.edge.parent.is_scene() and Candidate.is_primary(c)),
    Construction("scenes_and_scene_children", "scenes_and_scene_children",
                 lambda c: c.edge.child.is_scene() or c.edge.parent.is_scene() and Candidate.is_primary(c)),
    Categories(),
)
